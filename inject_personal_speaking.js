...
const regex = /speakingQuestions:\s*\[[\s\S]*?\],/g;
const matches = [...content.matchAll(regex)];
console.log('Found', matches.length, 'speakingQuestions blocks');

// Since we know the order of scenarios in the file matches the keys in scenarios.ts
// but it's safer to just split by id and replace the first speakingQuestions block after it.

for (const id in personalSpeakingData) {
  const jsonStr = JSON.stringify(personalSpeakingData[id], null, 4).replace(/\\n/g, '').replace(/^/gm, '    ');
  
  const idIndex = content.indexOf("id: '" + id + "'");
  if (idIndex !== -1) {
    const nextSpeakingQuestions = content.indexOf('speakingQuestions:', idIndex);
    const endOfArray = content.indexOf('],', nextSpeakingQuestions) + 2;
    
    const before = content.substring(0, nextSpeakingQuestions);
    const after = content.substring(endOfArray);
    
    content = before + 'speakingQuestions: ' + jsonStr.trim() + ',' + after;
  }
}

fs.writeFileSync('./data/scenarios.ts', content);
console.log('Successfully injected personal speaking questions');
