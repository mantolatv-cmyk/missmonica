...
for (let i = 0; i < lines.length; i++) {
  ...
  const trimmed = rawLine.trimEnd(); // removes \r and spaces at the end
  
  if (trimmed === '  },' || (trimmed === '  }' && i + 1 < lines.length && lines[i + 1].trimEnd() === '];')) {
    if (injectedCount < 18) {
      let originalBlock = readingBlocksData[injectedCount];
      
      originalBlock = originalBlock.replace(/questionsA1:\\s*\\[[\\s\\S]*?\\],\\s*questionsA2:/, replacements[injectedCount].A1 + ',\\n      questionsA2:');
      originalBlock = originalBlock.replace(/questionsA2:\\s*\\[[\\s\\S]*?\\]\\s*\\n\\s*\\}/, replacements[injectedCount].A2 + '\\n    }');
      
      const indentedBlock = '  ' + originalBlock.split('\\n').map((l, idx) => idx === 0 ? l : '  ' + l).join('\\n');
      
      newLines.push('    },');
      newLines.push(indentedBlock);
      injectedCount++;
      
      // We replaced the `  },` with `    },` + the block. Wait, the `  },` was closing the SCENARIO.
      // We want to add the reading block BEFORE `  },`.
      // So we push reading block, then we push `  },`!
      newLines.pop(); // remove indentedBlock
      newLines.pop(); // remove `    },`
      
      // Add a comma to the previous line if it doesn't have one!
      let prev = newLines.pop();
      if (!prev.trimEnd().endsWith(',') && !prev.trimEnd().endsWith('{') && !prev.trimEnd().endsWith('[')) {
        prev = prev.trimEnd() + ',';
      }
      newLines.push(prev);
      
      newLines.push(indentedBlock);
      newLines.push(rawLine); // push the `  },`
    } else {
...
fs.writeFileSync('data/scenarios.ts', newLines.join('\n'), 'utf8');
console.log('Successfully injected updated reading blocks. Count:', injectedCount);
