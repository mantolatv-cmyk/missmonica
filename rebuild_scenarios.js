...
if (!content.includes('export interface SpeakingQuestion')) {
  content = content.replace('export interface Scenario {', interfacesToAdd + '\\nexport interface Scenario {');
}

// 2. Add properties to Scenario interface
if (!content.includes('speakingQuestions?: SpeakingQuestion[];')) {
...
// 4. Prepare Reading Data
const readingBlocksData = JSON.parse(fs.readFileSync('scratch_reading.json', 'utf8'));

// 5. Inject everything safely
for (const id in personalSpeakingData) {
  const speakingArrStr = JSON.stringify(personalSpeakingData[id], null, 4).replace(/\\n/g, '').replace(/^/gm, '    ');
  
  let readingBlock = readingBlocksData[id];
  let readingStr = '';
  if (readingBlock) {
    readingStr = '  reading: ' + JSON.stringify(readingBlock, null, 4).replace(/\\n/g, '').replace(/^/gm, '    ') + ',\\n';
  }

  const replacement = "id: '" + id + "',\\n    speakingQuestions: " + speakingArrStr.trim() + ",\\n" + readingStr;
...
console.log('Successfully rebuilt scenarios.ts with all reading and speaking data!');
