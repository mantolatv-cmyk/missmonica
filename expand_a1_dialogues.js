...
for (const [id, variations] of Object.entries(extraLines)) {
  ...
  const insertExtra = (content, targetKey, linesToInject) => {
      let startIndex = 0;
      let newContent = content;
      
      while (true) {
          const idx = newContent.indexOf(targetKey + ': [', startIndex);
          if (idx === -1) break;
          
          const arrayStart = newContent.indexOf('[', idx);
          let openBrackets = 1;
          let closeIdx = -1;
          
          for (let i = arrayStart + 1; i < newContent.length; i++) {
              if (newContent[i] === '[') openBrackets++;
              if (newContent[i] === ']') {
                  openBrackets--;
                  if (openBrackets === 0) {
                      closeIdx = i;
          ...
          if (closeIdx !== -1) {
              const injectStr = JSON.stringify(linesToInject, null, 6).replace(/\"([a-zA-Z0-9_]+)\":/g, '$1:').slice(1, -1);
              
              const arrayContent = newContent.substring(arrayStart + 1, closeIdx).trim();
              const comma = arrayContent.length > 0 && !arrayContent.endsWith(',') ? ',' : '';
              
              newContent = newContent.substring(0, closeIdx) + comma + '\n' + injectStr + newContent.substring(closeIdx);
              startIndex = closeIdx + injectStr.length;
          } else {
              startIndex = idx + 1;
          }
      }
  ...
  scenarioContent = insertExtra(scenarioContent, 'dialoguesBeginner', variations[1]);
  scenarioContent = insertExtra(scenarioContent, 'dialoguesBeginner2', variations[2]);
  
  fileContent = fileContent.substring(0, scenarioStartIndex) + scenarioContent + fileContent.substring(searchScope);
  console.log(`Atualizado A1 dialogues em ${id}`);
}
